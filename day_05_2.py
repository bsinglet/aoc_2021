...
def plot_line_segments(parsed_lines):
	...
	for each_segment in parsed_lines:
		...
		if x1 == x2 and y1 != y2:
			...
			length = abs(x1 - x2) + 1
			# print(f"Printing {len(range(length))} points for this diagonal.")
			if x1 > x2:
				x1, x2 = x2, x1
				y1, y2 = y2, y1
			if y2 > y1:
				...
				for distance in range(length):
					sea_floor[(x1+distance, y1+distance)] += 1
			elif y2 < y1:
			...
			"""elif x2 < x1 and y2 > y1:
				print("Upper-right to Lower-left.")
				for distance in range(length):
					sea_floor[(x2+distance, y1+distance)] += 1
			elif x2 < x1 and y2 < y1:
				print("Lower-right to Upper-left")
				for distance in range(length):
                                        sea_floor[(x2+distance, y2+distance)] += 1
			"""
		else:
			# this is fine, we ignore lines that aren't vertical or horizontal
			print(f"Uhh, ({x1}, {y1}), ({x2}, {y2}) isn't a vertical, horizontal, or 45 degree line.")
			continue
		"""
		for row in range(10):
			print(''.join([str(sea_floor[(x, row)]) for x in range(10)]))
		"""
	return sea_floor
...
def main():
	...
	plot = list()
	for row in range(10):
		plot.append([str(sea_floor[(x, row)]) for x in range(10)])
		print(''.join(plot[row]))"""
